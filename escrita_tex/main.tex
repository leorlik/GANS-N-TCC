...
% título
\title{Geração de níveis para o jogo Super Mario Bros através da rede generativa adversarial MarioGAN}

% palavras-chave e keywords (p/ resumo, abstract e metadados do PDF)
\pchave{Geração de Conteúdo Procedural. MarioGAN. LVE.}
\keyword{Procedural Content Generation. MarioGAN. LVE.}

% autoria
\author{Leonardo Lima Dionizio}
\advisor{Eduardo J. Spinosa}
...
\include{3-relacionados/texto}			% estado da arte
\include{4-proposta/texto}		% proposta
\include{5-resultados/texto}		% experimentação e validação
\include{6-conclusao/texto}		% conclusão
%\include{7-apendice-niveis/texto}		% experimentação e validação
