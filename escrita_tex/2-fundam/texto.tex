...
O que este algoritmo faz é calcular uma Gaussiana com base na matriz de covariância estimada. Esta Gaussiana é traduzida numa elipse que se desloca de forma a contemplar as melhores soluções. Quanto mais distantes as melhores soluções estão do domínio dessa elipse, maior o \emph{step size} dela, ou seja, seu raio. No começo, o \emph{step size} costuma ser mais alto pois as melhores soluções conhecidas estão esparsas e a partir deste ponto de partida há a exploração global. Conforme a elipse fica melhor posicionada no domínio, as melhores soluções ficarão menos esparsas, fazendo com que o \emph{step size} diminua proporcionalmente com a concentração da população. O \textbf{Algoritmo 1} ilustra um pseudo-código do algoritmo CMA-ES.
...
\State Inicialize o \textit{step size}: $\sigma$
...
A busca A* possui múltiplas aplicações, entre elas a busca por caminhos viáveis em videogames. Até os dias de hoje, o mais popular algoritmo para o teste de jogabilidade de níveis do jogo \textit{Super Mario Bros} é o agente A* proposto por Robin Baumgarten em \cite{marioAI}.
...
Quanto as funções minimizadas em relação a topologia do nível minimizadas no artigo, as equações (2.1) e (2.2) as traduzem, em que g e t são respectivamente a fração da quantidade de blocos de chão na ultima linha da matriz que representa a fase e a fração desejada, e n o número de inimigos.
...
Em outros experimentos, os autores usam duas funções relacionadas ao teste pelo agente A* vencedor da competição da mario AI de 2009, conforme as equações (2.3) e (2.4), onde p é a fração da fase em que o agente passou, e \#jumps a quantidade de ações de pulos.
